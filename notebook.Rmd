...
$$
\frac{1}{\alpha}= \sum_{k=0}^\infty(1-\alpha)^k
$$

$$
x^n_{n+h} = x^\text{time for which you actually have data}_\text{time for which you′d like a forecast}
$$
$$
SES: x^n_{n+1} = \alpha x_n + (1-\alpha)x^{n-1}_{n}, \quad
\text{start with }x^1_2 = x_1
$$
...
$$
SES: \hat{x}_{n+1} = \alpha x_n + (1-\alpha)\hat{x}_{n}
$$


## Holt-Winters
Exponential smoothing with trend and seasonality or Triple exponential smoothing

* Levels, with parameter “alpha”, $\alpha$
* Trend, with parameter “beta”, $\beta$
* Seasonal Component , with parameter “gamma”, $\gamma$


getting a new level by updating the old level (forecast) and including new information.
new level = α ⋅ new information + (1 − α) ⋅ old level
$$
\hat{x}_{n+1} = \alpha x_n + (1-\alpha)\hat{x}_{n}
$$

Yesterday’s forecast plus a constant times the error in yesterday’s forecast
$$
\hat{x}_{n+1} = \hat{x}_{n} + \alpha (x_n - \hat{x}_{n})
$$

Level (l): smoothed (S) value of ts, t for time
$$
l_{t} = \alpha x_t + (1-\alpha)l_{t-1}
$$


$$
\hat{x}_{n+1} = \hat{l}_{n} = \alpha x_n + (1-\alpha)\hat{x}_{n}
$$

(double exponential) forecast = smoothed value + correction including trend information

The one-step-ahead equations for your forecast will look like

$$
\hat{x}_{n+1} = \text{level}_{n} + \text{trend}_{n}
$$
Smoothed value or level
$$
\text{level}_{n}  = \alpha \cdot  \text{new information} + (1-\alpha) (\text{old level} + \text{trend}) \\
\text{level}_{n}  = \alpha \cdot  x_n + (1-\alpha) (\text{level}_{n-1} + \text{trend}_{n-1})
$$

Trend: weighted average of the current change in level and previous trend, 
similarly to what we did for SES on time series values
$$
\text{trend}_{n-1} = \beta \cdot \text{new trend} + (1-\beta)\cdot\text{old trend} \\
\text{trend}_{n}  = \beta \cdot (\text{level}_{n} - \text{level}_{n-1}) + (1-\beta) \text{trend}_{n-1}
$$

Initial levels and trends then we can proceed inductively
$$
\text{level}_1 = x_1, \quad \text{trend}_1 = x_2 − x_1
$$
...
* strong seasonality
* trend
* heteroscedastic, with increasing variability in later years (see first plot below)
* transform log10(number of passengers): calmer looking plot with apparently additive seasonality

We saw that SES essentially led us to naïve forecasting.

$$
\hat{x}_{n+1} = \alpha x_n = x_n
$$
...
```{r}
hw <- HoltWinters(x = log10(AirPassengers))
hw
hw$SSE
```
...
(level_144 = hw$coefficients[1])  #a
(trend_144 = hw$coefficients[2])  #b
(seasonal_144_1_12 = hw$coefficients[2+horizon])  # s1

x144_1 = level_144 + horizon * trend_144 + seasonal_144_1_12
names(x144_1) <- c('x144_1')
x144_1
...
(seasonal_144_8_12 = hw$coefficients[2+horizon])  # s8

x144_8 = level_144 + horizon * trend_144 + seasonal_144_8_12
names(x144_8) <- c('x144_8')
x144_8
...
(seasonal_144_12_12 = hw$coefficients[2+horizon])  # s12

x144_12 = level_144 + horizon * trend_144 + seasonal_144_12_12
names(x144_12) <- c('x144_12')
x144_12
...
(seasonal_144_15_12 = hw$coefficients[2+horizon%%12])  # s3

x144_15 = level_144 + horizon * trend_144 + seasonal_144_15_12
names(x144_15) <- c('x144_15')
x144_15
...
```{r}
hw <- HoltWinters(x = log(AirPassengers))
fcast <- forecast(hw,24,lambda=0)
plot(fcast)
...
```{r}
hw <- HoltWinters(austres, gamma=FALSE)
hw

```

```{r}
x_n = tail(austres,1)[1]
alpha = hw$alpha
beta = hw$beta
h=1  # 1 quarter ahead

level_n = hw$coefficients[1]  #a
trend_n = hw$coefficients[2]  #b

# level_n = alpha*x_n + (1-alpha)*(level_n_1+trend_n_1)
# trend_n = beta *(level_n-level_n_1) + (1-beta) * trend_n_1

x_n_h = level_n + h * trend_n
names(x_n_h) <- paste0('x_n_',h)
x_n_h
```
...
```{r}
forecast(HoltWinters(austres, gamma=FALSE),1)
```
...
```{r}
hw <- HoltWinters(sunspots)
hw
```

```{r}
x_n = tail(sunspots,1)[1]
alpha = hw$alpha
beta = hw$beta
h=4  # 1 quarter ahead
...
level_n = hw$coefficients[1]  #a
trend_n = hw$coefficients[2]  #b
s_h = hw$coefficients[2+h]  #s1

# level_n = alpha*x_n + (1-alpha)*(level_n_1+trend_n_1)
# trend_n = beta *(level_n-level_n_1) + (1-beta) * trend_n_1

x_n_h = level_n + h * trend_n + s_h
names(x_n_h) <- paste0('x_n_',h)
x_n_h
```
...
df1 <- fortify(fcast) %>% as_tibble()

# Convert ts decimal time to Date class
df1$Date <- as.Date(date_decimal(df1$Index), "%Y-%m", tz="UTC")
str(df1)
...
Additive seasonality doesn’t care about the overall levels, 
whereas multiplicative seasonality does. 

A nice website to explore these differences is available from Nikolaos Kourentzes:
http://www.forsoc.net/2014/11/11/can-you-identify-additive-and-multiplicative-seasonality/

additive: January data add a constant to obtain the June projection
multiplicative: January data mupltiply by a constant to obtain the June projection


Additive seasonality
$$
\hat{x}_{n+h} = \text{level}_{n} + h\cdot\text{trend}_{n} + \text{seasonal}_{n+h-m}
$$

a one-step-ahead term called $\text{trend}_{n}$, we
move into the future ℎ steps in the simplest possible way by multiplying $\text{trend}_{n}$ by the number of time lags h we wish to look into the future (i.e. distance travelled is the size of
your step multiplied by the number of steps).


Multiplicative seasonality
$$
\hat{x}_{n+h} = (\text{level}_{n} + h\cdot\text{trend}_{n})\cdot \text{seasonal}_{n+h-m}
$$
...
## Forecast
1. Smooth the Level
weighted average of the seasonally adjusted level $x_n − s_{n−m}$ (we say we are
deseasonalizing) with the non-seasonal forecast
$$
 \text{level}_{n}  = \alpha \cdot (x_n -  \text{seasonal}_{n-m} ) + (1-\alpha) \cdot(\text{level}_{n-1} + \text{trend}_{n-1})
$$

2. Smooth the Trend
$$
\text{trend}_{n}  = \beta \cdot (\text{level}_{n} - \text{level}_{n-1}) + (1-\beta)\cdot \text{trend}_{n-1}
$$

3. Smooth the Season
$$
\text{seasonal}_{n}  = \gamma \cdot (x_{n} - \text{level}_{n-1}) + (1-\gamma)\cdot \text{seasonal}_{n-m}
$$
...
money.data <- subset(
  tsdl, description = "Volume of money, ABS definition m1. Feb 1960 – Dec 1994")
money.data.ts <- money.data[[1]]
...
Slow decay in the ACF, given the obvious trend
PACF seems to cut off very sharply
